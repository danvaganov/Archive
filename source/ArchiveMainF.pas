...
begin
    if (ArchiveDataModule.HasPermission('Archive.dbo.Организация', 'OBJECT', 'SELECT')) then
    begin
    ...
    begin
      raise Exception.Create('Недостаточно прав для просмотра справочника абонентов');
    end
end;
...
begin
  ...
  begin
    raise Exception.Create('Недостаточно прав для просмотра журнала документов');
  end
end;

procedure TArchiveMainForm.GuideKindDocumentExecute(Sender: TObject);
begin
    if (ArchiveDataModule.HasPermission('Archive.dbo.Вид_Документ', 'OBJECT', 'SELECT')) then
    begin
    ...
    begin
      raise Exception.Create('Недостаточно прав для просмотра справочника видов документа');
    end
end;
